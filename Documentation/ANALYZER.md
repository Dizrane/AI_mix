...
Project metadata is caption-gated: a value becomes a `known` fact only when a control's own AX caption is the label (exactly, or as a whole-word prefix) and the control itself exposes an `AXValue` — a caption is a label and is never published as its own value. The project name is not searched for at all, because no Logic control is captioned "project": it comes from the main window (`AXMainWindow`), preferring the `AXDocument` file URL (the `.logicx` name) and falling back to the window's `AXTitle` verbatim; with neither attribute the name stays `unavailable`.
