...
2. **Inputs.** Each track is an `AVAudioFile` scheduled on its own `AVAudioPlayerNode` at t=0 (plus the alignment offset of step 7 when other chains are slower). The mix sample rate is the first file's rate; a file at any other rate is a named error — this prototype never resamples. Mono and stereo inputs are accepted (a mono source is upmixed by the mixer node's own pan law).
...
7. **Latency compensation.** Insert latency is compensated sample-accurately, from numbers the units themselves report and never from guesswork. Each insert's `auAudioUnit.latency` is read only after `engine.start()` has allocated render resources — the one moment the number is real — and handed to the pure planning math in `Sources/MixLatency.swift` (`MixLatencyMath`). The alignment is exact by construction: every input WAV starts at t=0, so a track whose insert chain is faster than the slowest one simply starts later — its player renders exactly `maxTrackChain − thisChain` frames of scheduled silence before the file, which aligns every track-mixer output BEFORE any send fan-out and therefore needs no DSP in any track's path. A send's two branches (direct to the master sum, and through a bus whose inserts add latency) diverge after that point and no offset can split a fan-out, so the divergence is repaired where it happens: the direct branch runs through a delay of `maxBusChain` frames and each bus output through `maxBusChain − thisBus` frames, making every path into the master sum equally late. These branch delays are the engine's own `SampleExactDelayAudioUnit` — a whole-frame ring that stores and replays samples untouched, bit-exact by construction (`AVAudioUnitDelay` was rejected deliberately: its wet path carries a low-pass filter and fractional-time interpolation, coloration that would have to be proven away rather than excluded by design). The graph's total latency (slowest track chain + slowest bus chain + master chain) is rendered in addition to the mix length and trimmed from the head of the file, so `mix.wav` stays positionally aligned to t=0 with its full tail. Every insert's reported latency lands in `MixInsertReport` (seconds and frames), the total in `MixRenderResult.compensatedLatencyFrames`; anything that cannot be compensated — a unit reporting a fractional or negative latency — is named in `notes` instead of being silently misaligned. A graph whose units all report zero latency is a provable no-op: no offsets, no delay nodes (they exist only when a bus carries inserts), no trim.
8. **Render.** The engine pulls `renderOffline` in ≤4096-frame blocks for the longest input's length plus `tailSeconds` (so reverb/delay tails are captured) plus the latency trim, drops the trim from the head, and writes `mix.wav` as float32 or 24-bit PCM at the input sample rate.
9. **Measurement.** The finished file is measured by the same local `AudioMetricsAnalyzer` the app uses for exported WAVs (BS.1770-4 LUFS, true peak, sample peak, clipped-sample count, spectrum…); `MixRenderResult` carries the numbers and the insert reports. The engine only reports facts — what is acceptable is the caller's judgement. Bus outputs can additionally be measured through render taps, explicitly best-effort: manual-rendering taps carry no delivery guarantee, so a bus is either measured or named unavailable in `notes`, never guessed.
...
CI runners have only Apple's built-in AUs, so the suite (`Tests/MixEngineTests.swift`) proves the engine on AUParametricEQ, AUMatrixReverb and AUPeakLimiter: exact arithmetic summing with positional alignment, parameter roundtrip through the tree plus a real spectral change in the cut band, an audible reverb tail through the send topology, zero clipped samples through a master limiter on a deliberately clipping sum (which now also proves the render is NOT shifted by the limiter's look-ahead), and named errors for missing plugins, unresolved parameters, mismatched sample rates and unknown buses. Latency compensation is proven at both layers: the planning math is unit-tested on synthetic latencies (unequal track chains, sends into latent buses, a latent master chain, and the zero-latency no-op), the delay ring is proven bit-exact with plain float equality, and the rendered proofs never hardcode any plugin's latency — each test reads the latency the unit itself reports at runtime and derives its expectations from it, so they hold exactly whether the installed limiter reports look-ahead or zero. An impulse through a latent track chain lands at exactly the input sample and sums arithmetically with a parallel clean track at that same sample; a send into a latent bus adds its energy at exactly the direct branch's sample; the direct path through the alignment delay comes out faithful with everything else silent. Rendering a project's exported WAVs through third-party AUs (FabFilter etc.) exercises the same code paths but must be smoke-tested on a real Mac with those plugins installed.
