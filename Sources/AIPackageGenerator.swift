...
struct AIPackageGenerator: Sendable {
    ...
    private func mixPlanSchemaSection() -> [String] {
        ["", "## Mix Plan schema (machine-validated)", "", "The application validates the pasted Mix Plan technically against the current snapshot — targets must exist in the facts above, every action must carry its typed `parameters.value`, and plug-in parameter values must sit inside their reported ranges. It never judges musical merit, and nothing is written to Logic Pro.", "", "- Every field shown in the example is required on every action, and every action carries a `parameters.value`.", "- `action` must be one of: `set_volume`, `set_pan`, `set_mute`, `set_solo`, `set_plugin_bypass`, `set_plugin_parameter`. No other action is implemented; anything else is rejected as unsupported. In particular there is NO action that adds a plug-in — plug-in recommendations stay prose for the user to apply manually.", "- `set_volume` and `set_pan` take a numeric `parameters.value`: `set_volume` in dB, the same scale as the Volume channel facts above; `set_pan` in Logic's pan units as shown in the Pan facts (0 = centre, negative = left, positive = right). `set_mute` and `set_solo` take a boolean `parameters.value`. A missing or mistyped value is rejected as invalid.", "- Target a track with `target.trackID` = the `logicalTrackID` used throughout this document, or `target.trackName` = the exact Logic track name.", "- `set_plugin_bypass` (boolean `parameters.value`) and `set_plugin_parameter` additionally need `target.pluginID` or `target.pluginName`, and `set_plugin_parameter` needs `target.parameterID` or `target.parameterName` plus a numeric `parameters.value`. When no plug-in facts were captured for the track, these validate as `requires_probe`, not as executable actions.", "", "```json", Self.mixPlanExampleJSON, "```"]
    }
