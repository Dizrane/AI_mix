    ...
    private func trackLinkingSection(_ snapshot: NormalizedSnapshot) -> [String] {
        ...
        for track in snapshot.tracks {
            ...
            if let channel = track.channel {
                ...
                out += ["", "##### Routing"] + compact([entry("Input", channel.input), entry("Output", channel.output)]) + ["", "##### Routing buttons (slot kind unclassified)"]
                out += channel.routingButtons.isEmpty ? ["- unavailable: no routing buttons captured"] : channel.routingButtons.flatMap { ["- Routing button `\($0.id)`", "  - Destination: \(render($0.destination))", "  - Slot kind (send / output / aux input): requires_probe"] }
                out += ["", "##### Plugins"]
