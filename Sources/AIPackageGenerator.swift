...
struct AIPackageGenerator: Sendable {
    ...
    private func trackLinkingSection(_ snapshot: NormalizedSnapshot) -> [String] {
        ...
        for track in snapshot.tracks {
            ...
            if let channel = track.channel {
                ...
                out += ["", "##### Routing"] + compact([routingEntry("Input", channel.input), routingEntry("Output", channel.output)]) + ["", "##### Sends"]
                out += channel.sends.isEmpty ? ["- none: no occupied send slot on this strip (an occupied send is structurally unmistakable, so this is a fact, not missing data)"] : channel.sends.flatMap { ["- Send `\($0.id)`", "  - Destination: \(render($0.destination))\(kindSuffix($0.destination))", "  - Bypass: \(render($0.bypass))", "  - Level: requires_probe (the send knob exposes only a unitless raw value)", "  - Pan: requires_probe (no send pan control is exposed)"] }
                out += ["", "##### Routing buttons (slot kind unclassified)"]
                out += channel.routingButtons.isEmpty ? ["- none: no destination button on this strip is left unclassified"] : channel.routingButtons.flatMap { ["- Routing button `\($0.id)`", "  - Destination: \(render($0.destination))\(kindSuffix($0.destination))", "  - Slot kind (send / output / aux input): requires_probe"] }
                out += ["", "##### Plugins"]
    ...
    private func mixSection(_ mix: MixBounceAsset?, delivery: PackageDelivery) -> [String] {
        ...
        if let s = mix.bounceSettings {
            ...
            if let formats = s.formats.value { out += ["- Bounce dialog formats (read from Logic's own format table; the app checks the PCM row itself when none is checked, and cancels only when that check fails): known: \(formats.caption)"] }
