...
struct AIPackageGenerator: Sendable {
    ...
    private func provenanceSection(_ assets: [AudioAsset]) -> [String] {
        var out = ["", "## Audio ↔ Logic provenance", "", "Each WAV maps unambiguously to one Logic audio track (via logicalTrackID) and its exact Logic name. Only audio tracks have WAVs; Aux/Bus/Master/Output channel-only objects do not appear here. Region IDs are context only, never separate files."]
        ...
        out += [""] + assets.map { asset in
            ...
            return "- \(wav) ← `\(asset.logicalTrackID)` \u{201C}\(asset.trackName.value ?? "unknown")\u{201D} • \(plural(asset.regionCount, "region"))\(asset.regionIDs.isEmpty ? "" : ": " + asset.regionIDs.joined(separator: ", "))"
        }
        return out
    }
