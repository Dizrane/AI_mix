...
struct AIPackageGenerator: Sendable {
    ...
    func make(snapshot: NormalizedSnapshot, sessionID: String, audio: [AudioAsset] = [], plugins: [PluginInventoryItem] = [], probes: [ProbeType] = ProbeType.allCases, delivery: PackageDelivery = .fullPackage, exportSettings: ExportSettingsFacts? = nil, mix: MixBounceAsset? = nil, postApply: PostApplyReport? = nil) -> String {
        ...
        // The two response contracts are mutually exclusive deliverables: the Assistant path renders offline from a
        // MixGraph, the manual paths validate a MixPlan on the Review screen — teaching both in one document would
        // invite the model to deliver the wrong one.
        out += delivery == .apiDelivery ? mixGraphSchemaSection(assets: audio, plugins: plugins) : mixPlanSchemaSection(snapshot)
    ...
    private func trackLinkingSection(_ snapshot: NormalizedSnapshot) -> [String] {
        ...
        for track in snapshot.tracks {
            ...
            if let channel = track.channel {
                ...
                out += channel.sends.isEmpty ? ["- Sends: none"] : ["", "##### Sends"] + channel.sends.flatMap { ["- Send `\($0.id)`", "  - Destination: \(render($0.destination))\(kindSuffix($0.destination))", "  - Bypass: \(render($0.bypass))", "  - Level: \(sendLevelLine($0))", "  - Pan: requires_probe (no send pan control is exposed — `set_send_pan` is never executable; send pan changes belong in MANUAL STEPS)"] }
    ...
    private func sendLevelLine(_ send: SendFacts) -> String {
        guard let value = send.level.value, let scale = send.levelScale else { return "\(send.level.state.rawValue) (the knob proves no scale — no dB AXValueDescription and no bounded numeric AXValue — so `set_send_level` stays unsupported for this send)" }
    ...
    private func currentControlValuesSection(_ snapshot: NormalizedSnapshot) -> [String] {
        ...
        if !sendRows.isEmpty {
            out += ["", "### Send levels (source for `parameters.current` of `set_send_level`)", "", "One row per occupied send. `set_send_level` targets a send with `target.trackID` (or `target.trackName`) plus `target.sendDestination` copied exactly from the Destination column. Copy `parameters.current` from the Level column exactly; the Scale column names the proven scale both `current` and your `parameters.value` are on — `dB`, or `raw knob units` when Logic exposes no dB for that knob (the number is the knob's own position, not decibels). A Level cell that names a state instead of a number means the knob's scale is unproven: `set_send_level` validates as unsupported there — keep that move in MANUAL STEPS instead.", "", "| logicalTrackID | Send destination | Level | Scale | Proven range |", "| --- | --- | --- | --- | --- |"]
