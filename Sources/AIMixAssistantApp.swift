    ...
    var body: some View {
        Screen(title: "Audio", subtitle: "One WAV per Logic track. Regions are preserved as provenance, never split.") {
            ...
            Card {
                ...
                Text("Bounces the whole project through the master chain via Logic's own File \u{25B8} Bounce dialog into the app's mix folder, then measures the file. The sum is the reference for overall loudness and balance — per-track WAVs never contain it. Only Normalize is ever changed: a level-rewriting value is switched to Off automatically; everything else is read as facts.").font(.caption).foregroundStyle(.secondary)
            }
