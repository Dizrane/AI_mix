    ...
    var body: some View {
        Screen(title: "Audio", subtitle: "One WAV per Logic track. Regions are preserved as provenance, never split.") {
            ...
            Card {
                ...
                Text("Bounces the whole project through the master chain via Logic's own File \u{25B8} Bounce dialog into the app's mix folder, then measures the file. The sum is the reference for overall loudness and balance — per-track WAVs never contain it, so the AI Package stage opens only after both the track export and this bounce are done. Only three settings are ever corrected automatically: an enabled Cycle is switched off so the bounce covers the whole project rather than the cycle section, a level-rewriting Normalize is switched to Off, and the format table is set to PCM (Uncompressed) alone — compressed formats are unchecked; everything else is read as facts. The bounce file is measured and accepted as is — any duration mismatch against the exported tracks is published as a fact in the AI Package, never a reason to reject the file.").font(.caption).foregroundStyle(.secondary)
            }
