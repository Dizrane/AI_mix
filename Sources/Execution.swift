...
struct DiffEngine: Sendable { func compare(before: NormalizedSnapshot, after: NormalizedSnapshot) -> SnapshotDiff {
    ...
    // Sends compare by content, not by encoded facts: send ids and sources embed AX paths, which legally shift
    // between scans — only the destination, bypass and level values say whether a send really changed.
    func sendSignature(_ sends: [SendFacts]?) -> [[String?]] { (sends ?? []).map { [$0.destination.value, $0.bypass.value.map(String.init), $0.level.value.map { String($0) }] } }
    for (id, old) in beforeTracks { guard let new = afterTracks[id] else { changed.append("Track removed: \(old.name.value ?? id)"); continue }; let pluginsChanged = old.channel?.plugins != new.channel?.plugins; let sendsChanged = sendSignature(old.channel?.sends) != sendSignature(new.channel?.sends); if old.channel?.volumeDB.value != new.channel?.volumeDB.value || old.channel?.pan.value != new.channel?.pan.value || old.channel?.mute.value != new.channel?.mute.value || old.channel?.solo.value != new.channel?.solo.value || pluginsChanged || sendsChanged { changed.append("Changed: \(old.name.value ?? id)") } else { unchanged.append(old.name.value ?? id) } }
