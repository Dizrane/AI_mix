...
struct LogicAccessibilityAnalyzer: DAWAnalyzer {
    ...
    private func inspect(_ element: AXUIElement, path: String, depth: Int, visited: inout Int, progress: (Int) -> Void) throws -> RawAccessibilityNode {
        ...
        return .init(id: path, role: role, subrole: attrs.subrole, title: attrs.title, description: attrs.description, value: attrs.value, enabled: attrs.enabled, position: attrs.position, size: attrs.size, supportedAttributes: attributeNames(element), parameterizedAttributes: parameterizedAttributeNames(element), actions: actionNames(element), children: children, valueDescription: attrs.valueDescription, minValue: attrs.minValue, maxValue: attrs.maxValue)
    }
    private struct NodeAttributes { var role: String?; var subrole: String?; var title: String?; var description: String?; var value: String?; var enabled: Bool?; var position: String?; var size: String?; var valueDescription: String?; var minValue: String?; var maxValue: String? }
    private func batchAttributes(_ element: AXUIElement) -> NodeAttributes {
        let keys = [kAXRoleAttribute, kAXSubroleAttribute, kAXTitleAttribute, kAXDescriptionAttribute, kAXValueAttribute, kAXEnabledAttribute, kAXPositionAttribute, kAXSizeAttribute, kAXValueDescriptionAttribute, kAXMinValueAttribute, kAXMaxValueAttribute]
        var raw: CFArray?
        guard AXUIElementCopyMultipleAttributeValues(element, keys as CFArray, [], &raw) == .success, let values = raw as? [AnyObject], values.count == keys.count else {
            return NodeAttributes(role: string(element, kAXRoleAttribute), subrole: string(element, kAXSubroleAttribute), title: string(element, kAXTitleAttribute), description: string(element, kAXDescriptionAttribute), value: valueString(attribute(element, kAXValueAttribute)), enabled: attribute(element, kAXEnabledAttribute) as? Bool, position: axValueString(attribute(element, kAXPositionAttribute), type: kAXValueCGPointType), size: axValueString(attribute(element, kAXSizeAttribute), type: kAXValueCGSizeType), valueDescription: string(element, kAXValueDescriptionAttribute), minValue: valueString(attribute(element, kAXMinValueAttribute)), maxValue: valueString(attribute(element, kAXMaxValueAttribute)))
        }
        ...
        return NodeAttributes(role: present(0) as? String, subrole: present(1) as? String, title: present(2) as? String, description: present(3) as? String, value: valueString(present(4)), enabled: present(5) as? Bool, position: axValueString(present(6), type: kAXValueCGPointType), size: axValueString(present(7), type: kAXValueCGSizeType), valueDescription: present(8) as? String, minValue: valueString(present(9)), maxValue: valueString(present(10)))
    }
